...
class BERTBaseChineseModel(BaseModel):
    """
    ...
    def __init__(self, opt, device, vocab, side_data):
        ...
        resource = resources['bert_base_chinese'][language]
        dpath = os.path.join(PRETRAIN_PATH, "bert_base_chinese", language)
        super(BERTBaseChineseModel, self).__init__(opt, device, dpath, resource)

    def build_model(self):
        """build model"""
        self.model = AutoModelForMaskedLM.from_pretrained(self.dpath)
        self.loss = CrossEntropyLoss(ignore_index=self.pad_id)

    def forward(self, batch, mode):
        _, _, input_ids, context, _, _, y = batch
        if mode != 'test':
            # torch.tens  or's shape = (bs, seq_len, v_s); tuple's length = 12
    ...
    def generate(self, context):
        ...
        a = self.model.generate(context, max_new_tokens=self.response_truncate, num_beams=4, num_return_sequences=1, return_dict_in_generate=True)
        b = a["sequences"][:,self.context_truncate:]
        return b
