...
class StandardEvaluator(BaseEvaluator):
        ...
        if hyp:
            ...
            for k in range(1, 5):
                ...
                for token in ngrams(hyp, k):
                    ...
                    # cyf fixed 2023.02.03
                    self.dist_cnt[k - 1] += 1
            # older
            # self.dist_cnt[k - 1] += 1

            # cyf fixed 2023.02.10. 不然若是没有4-gram,那么dist@4索引就会报错
            for k in range(1, 5):
                self.dist_set[f"dist@{k}"] = self.dist_set.get(f"dist@{k}", set())
    ...
    def report(self, epoch=-1, mode='test'):
        ...
        for k, v in self.dist_set.items():
            if self.dist_cnt[temp_count] != 0:
                self.gen_metrics.add(k, AverageMetric(len(v) / self.dist_cnt[temp_count]))
            else:
                self.gen_metrics.add(k, AverageMetric(0))
            temp_count += 1
