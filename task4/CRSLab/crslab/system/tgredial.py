...
from crslab.data import get_dataloader, dataset_language_map, TGReDialDataLoader, KGSFDataLoader
...
class TGReDialSystem(BaseSystem):
    ...
    def __init__(self, opt, train_dataloader, valid_dataloader, test_dataloader, vocab, side_data, restore_system=False,
        ...
        """
        super(TGReDialSystem, self).__init__(opt, train_dataloader, valid_dataloader, test_dataloader, vocab, side_data, restore_system, interact, debug, tensorboard)
        ...
        self.wandb = wandb.init(project="TGRedial System", config=self.opt.opt)
    ...
    def step(self, batch, stage, mode):
        ...
        if stage == 'policy':
            ...
            if isinstance(policy_loss, torch.Tensor):
                ...
                self.evaluator.optim_metrics.add("policy_loss", AverageMetric(policy_loss))
        elif stage == 'rec':
            if mode == 'train':
            ...
            if mode != "test":
                ...
                if mode == 'train':
                    self.backward(gen_loss)  # 更新参数
                else:
                    self.conv_evaluate(pred, batch[-1])
                gen_loss = gen_loss.item()  # 训练集也评估一下,收集一下gen_loss和ppl
                self.evaluator.optim_metrics.add("gen_loss", AverageMetric(gen_loss))  # 每一个batch这里evaluator的gen_loss,ppl都是会变化的
                ...
                context = None
                response = None
                if isinstance(self.test_dataloader["conv"], TGReDialDataLoader):
                    context = batch[3]
                    response = batch[-1]
                elif isinstance(self.test_dataloader["conv"], KGSFDataLoader):
                    context = batch[0]
                    response = batch[-1]
                else:
                    print("Error at TGRedial System")
                    exit(0)

                for c, p, r in zip(context, pred, response):
                    c_str = ind2txt(c, self.ind2tok, self.end_token_idx)
                    p_str = ind2txt(p, self.ind2tok, self.end_token_idx)
    ...
    def train_recommender(self):
        ...
        with torch.no_grad():
            self.evaluator.reset_metrics()
            for batch in self.test_dataloader['rec'].get_rec_data(self.rec_batch_size, shuffle=False):
    ...
    def train_conversation(self):
        ...
        for epoch in range(self.conv_epoch):
            ...
            for batch in self.train_dataloader['conv'].get_conv_data(batch_size=self.conv_batch_size, shuffle=True):
