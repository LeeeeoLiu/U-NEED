...
class UNEODDataset(BaseDataset):
    """
    ...
    def __init__(self, opt, tokenize, restore=False, save=False):
        ...
        dpath = os.path.join(DATASET_PATH, "uneod", tokenize)
    ...
    def _load_other_data(self):
        self.entity2id = json.load(open(os.path.join(self.dpath, 'entity2id.json'), 'r', encoding='utf-8'))  # {entity: entity_id}
        self.id2entity = {idx: entity for entity, idx in self.entity2id.items()}
        self.n_entity = len(self.entity2id)
        # {head_entity_id: [(relation_id, tail_entity_id)]}
        self.entity_kg = json.load(open(os.path.join(self.dpath, 'subkg.json'), 'r', encoding='utf-8'))
        logger.debug(f"[Load entity dictionary and KG from {os.path.join(self.dpath, 'entity2id.json')} and {os.path.join(self.dpath, 'subkg.json')}]")
        ...
        logger.debug(f"[Load word dictionary and KG from {os.path.join(self.dpath, 'concept2id.json')} and {os.path.join(self.dpath, 'conceptnet_subkg.txt')}]")
    ...
    # 开始构建context,即从第二句话开始,都以这句话为回复,以之前的话为对话内容,对话内容里面每一句话都是一个列表会分隔开,而entity,item,word就不会,直接一个list装就可以
    ...
    def _entity_kg_process(self, SELF_LOOP_ID=-1):
