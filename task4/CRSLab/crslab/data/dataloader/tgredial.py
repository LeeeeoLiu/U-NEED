...
class TGReDialDataLoader(BaseDataLoader):
    ...
    # 注意所有DataLoader类里面, 都有conv_process_fn, 在训练过程中，get_conv_data->get_data，而get_data会以conv_process_fn函数作为一个参数，BaseDataLoader里面会返回全部的Dataset,而KGSFDataLoader只会返回回复是Recommender的，TGRedialDataLoader没有重写这个方法，就采用了BaseDataLoader.
    # 不仅对话，推荐和策略也是相同的原理
    # cyf fixed 2023.02.10
