...
class dataset(object):
    def __init__(self, filename, opt):
        self.entity2entityId = pkl.load(open('data/entity2entityId.pkl', 'rb'))  # entity_name(str) : index(int)
        self.movieID2selection_label = pkl.load(open('data/movieID2selection_label.pkl', 'rb'))  # movie_id(int) : selection_label(int)
        ...
        self.id2entity = pkl.load(open('data/id2entity.pkl', 'rb'))  # movie_id(int) : enetity_name(str)
        self.subkg = pkl.load(open('data/subkg.pkl', 'rb'))  # need not back process
        self.text_dict = pkl.load(open('data/text_dict.pkl', 'rb'))  # text(str,the dialogue sentences) : [](a list)
    ...
    def padding_w2v(self, sentence, max_length, transformer=True, is_response=False, movies_gth=None, pad=0, end=2, unk=3):
        ...
        if movies_gth is not None:
            # print('cur_movie:', cur_movie)
            # print('len(movies_gth):', len(movies_gth))
            assert cur_movie == len(movies_gth)
        ...
        if len(vector) > max_length:
            if transformer:
                movie_nums = sum(movie_mask[-max_length:])
                if movie_nums != 0 and movies_gth is not None:
                    movies_gth = movies_gth[-movie_nums:]
                else:
                    movies_gth = []
                return vector[-max_length:], max_length, concept_mask[-max_length:], dbpedia_mask[
                                                                                     -max_length:], movies_gth
            else:
                movie_nums = sum(movie_mask[:max_length])
                movies_gth = movies_gth[:movie_nums]
    ...
    def data_process(self, is_finetune=False):
        ...
        for line in self.data:
            ...
            response, r_length, _, _, movies_gth = self.padding_w2v(line['response'], self.max_r_length,
                                                                    transformer=True, is_response=True,
                                                                    movies_gth=line['all_movies'])
            ...
            if False:
                mask_response, mask_r_length, _, _, _ = self.padding_w2v(self.response_delibration(line['response']),
                                                                         self.max_r_length)
            else:
    ...
    def detect_movie(self, sentence, movies):
        ...
        token_text = list(jieba.cut(sentence))
        num = 0
        token_text_com = []
        ...
        for i in range(len(token_text_com)):
            if token_text_com[i][1:] in movies:
                ...
                all_movie_selection_label.append(self.movieID2selection_label[token_text_com[i][1:]])
    ...
    def _context_reformulate(self, context, movies, altitude, ini_altitude, s_id, re_id):
        ...
        for message in context:
            ...
            if len(context_list) == 0:
                ...
                context_dict = {'text': token_text, 'entity': entities + movie_rec, 'user': message['senderWorkerId'],
                                'movie': movie_rec, 'movies_selection_labels': all_movie_selection_label}
            ...
            if context_dict['user'] == re_id and len(contexts) > 0:
                ...
                if len(context_dict['movie']) != 0:
                    for movie in context_dict['movie']:
                        ...
                        cases.append(
                            {'contexts': deepcopy(contexts), 'response': response, 'entity': deepcopy(entities),
                             'movie': movie, 'rec': 1, 'all_movies': context_dict['movies_selection_labels']})
                        # cases[-1]['all_movies']=context_dict['movie']
                else:
                    cases.append(
                        {'contexts': deepcopy(contexts), 'response': response, 'entity': deepcopy(entities), 'movie': 0,
                         'rec': 0, 'all_movies': context_dict['movies_selection_labels']})
...
class CRSdataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return context, c_lengths, response, r_length, mask_response, mask_r_length, entity_vec, entity_vector, movie, np.array(
            concept_mask), np.array(dbpedia_mask), concept_vec, db_vec, rec, movies_gth, movies_num
