from models.transformer import TorchGeneratorModel, _build_encoder, _build_decoder, _build_encoder_mask, \
    _build_encoder4kg, _build_decoder4kg, _build_decoder_selection
...
class CrossModel(nn.Module):
    ...
    def forward(self, xs, ys, mask_ys, concept_mask, db_mask, seed_sets, labels, con_label, db_label, entity_vector,
                rec, movies_gth=None, movie_nums=None, test=True, cand_params=None, prev_enc=None, maxlen=None,
                bsz=None):
        """
        ...
        for i, seed_set in enumerate(seed_sets):
            ...
            # db:dbpedia:item space,con:concept:word space
            # torch.ones生成长度为size的值全为一的张量
            db_con_mask.append(torch.ones([1]))

        # 用户表示不是一个随机的item,而是很多个随机的item的叠加,但是会不会是理解错了,这个意思是很多个用户的表示?
        db_user_emb = torch.stack(user_representation_list)
        db_con_mask = torch.stack(db_con_mask)

        # concept也是一个知识图谱，这个concept_mask到底是什么意思，类型是一个int吧
        ...
        # 啊这,这个不会也是一个变量影响生成word还是item吧,不过这里是两个知识图谱得到的用户表示的叠加。
        user_emb = uc_gate * db_user_emb + (1 - uc_gate) * con_user_emb
        # 每一个实体也会有一个得分
        entity_scores = F.linear(user_emb, db_nodes_features, self.output_en.bias)
        # 这一部分应该是两种用户表示融合,计算得到一个用户表示,然后用户表示与item计算相似度,在NTRD中应该只是将找到的商品作为候选集合,供后续的item selector选择
        ...
        # 两个用户的表示之间的差,或者说item embedding之间的差?
        info_db_loss, info_con_loss = self.infomax_loss(con_nodes_features, db_nodes_features, con_user_emb,
                                                        db_user_emb, con_label, db_label, db_con_mask)

        # entity_scores = F.softmax(entity_scores.cuda(), dim=-1).cuda()

        # 推荐上的loss,就是之前计算出来的分数,与真实值之间的差距
        ...
        if self.is_finetune:
            # generation---------------------------------------------------------------------------------------------------
            encoder_states = prev_enc if prev_enc is not None else self.encoder(xs)  # 如果之前有计算过那就直接用上次的,不然那就重新便编码
            # concept for generation,word space,concept KG中的节点
            ...
            # dbpedia for generation,item space
            ...
            if test == False:
                ...
                temp = movies_gth
                # 有时候movies_gth出现了0,然后经过这个操作之后就count--了,因为全部-1都变成0把0淹没了
                ...
                a = torch.sum(temp != -1, dim=(0, 1))
                b = torch.sum((mask_ys == MOVIE_TOKEN_INDEX), dim=(0, 1))
                if a != b:
                    print("hello")
                assert a == b
                ...
                masked_for_selection_token = (mask_ys == MOVIE_TOKEN_INDEX)
                ...
                movies_gth = torch.masked_select(movies_gth, (temp != -1))
                selection_loss = torch.mean(
                    self.compute_loss(matching_logits, movies_gth))  # movies_gth.squeeze(0):[bsz * dynamic_movie_nums]
            else:
                # ---------------------------------------------Beam Search decode----------------------------------------
                ...
                # #-------------------------------- stage2 movie selection loss-------------- by Jokie
                preds_for_selection = preds[:, 1:]  # skip the start_ind
                masked_for_selection_token = (preds_for_selection == MOVIE_TOKEN_INDEX)
