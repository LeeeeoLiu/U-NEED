...
def _build_decoder_selection(opt, dictionary_len, padding_idx=None,
                             n_positions=1024):
    return TransformerDecoderSelection(
        n_heads=opt['n_heads'],
        n_layers=opt['n_layers'],
        embedding_size=opt['embedding_size'],
        ffn_size=opt['ffn_size'],
        vocabulary_size=dictionary_len,
        # embedding=embedding,
        dropout=opt['dropout'],
        attention_dropout=opt['attention_dropout'],
        relu_dropout=opt['relu_dropout'],
...
def _build_decoder_e2e_selection(opt, dictionary_len, padding_idx=None,
                                 n_positions=1024):
    return TransformerDecoderE2ESelection(
        n_heads=opt['n_heads'],
        n_layers=opt['n_layers'],
        embedding_size=opt['embedding_size'],
        ffn_size=opt['ffn_size'],
...
class TransformerDecoderLayerSelection(nn.Module):
    def __init__(
        ...
        self.norm2_movie = nn.LayerNorm(embedding_size)

        self.decoder_stage1_attention = MultiHeadAttention(
            n_heads, embedding_size, dropout=attention_dropout
        )
        self.norm2_de = nn.LayerNorm(embedding_size)
    ...
    def forward(self, x, encoder_output, encoder_mask, movie_embed, movie_embed_mask):
        ...
        # residual = x
        # x = self.decoder_stage1_attention(
        #     query=x,
        #     key=decoder_output,
        #     value=decoder_output,
        #     mask=decoder_output_mask
        # )
        # x = self.dropout(x)  # --dropout
        # x = residual + x
        # x = _normalize(x, self.norm2_de)
        ...
        x = self.dropout(x)  # --dropout
        x = residual + x
        x = _normalize(x, self.norm2_movie)
...
class TransformerDecoderSelection(nn.Module):
    ...
    def forward(self, input, encoder_state, movie_embed_state, incr_state=None):
        # def forward(self, input, encoder_state, movie_embed_state, decoder_stage1_state, incr_state=None):
        encoder_output, encoder_mask = encoder_state
        movie_embed, movie_embed_mask = movie_embed_state
        # decoder_stage1_output, decoder_stage1_mask = decoder_stage1_state
        ...
        #     tensor = tensor * np.sqrt(self.dim)
        # tensor = tensor + self.position_embeddings(positions).expand_as(tensor)
        # tensor = self.dropout(input)  # --dropout
        ...
        for layer in self.layers:
            ...
            # tensor = layer(tensor, encoder_output, encoder_mask, movie_embed, movie_embed_mask, decoder_stage1_output, decoder_stage1_mask)

        return tensor, None


class TransformerDecoderLayerE2ESelection(nn.Module):
    def __init__(
            self,
            n_heads,
            embedding_size,
        ...
        self.norm2_con = nn.LayerNorm(embedding_size)

        self.ffn = TransformerFFN(embedding_size, ffn_size, relu_dropout=relu_dropout)
        self.norm3 = nn.LayerNorm(embedding_size)

    def forward(self, x, encoder_output, encoder_mask, movie_embed, movie_embed_mask, concept_embed, concept_embed_mask):
        # def forward(self, x, encoder_output, encoder_mask, movie_embed, movie_embed_mask, decoder_output, decoder_output_mask):
        ...
        residual = x
        x = self.concept_attention(
            query=x,
            key=concept_embed,
            value=concept_embed,
            mask=concept_embed_mask
        )
        x = self.dropout(x)  # --dropout
        x = residual + x
        x = _normalize(x, self.norm2_con)
...
class TransformerDecoderE2ESelection(nn.Module):
    """
    Transformer Decoder layer.
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList()
        for _ in range(self.n_layers):
            self.layers.append(TransformerDecoderLayerE2ESelection(
                n_heads, embedding_size, ffn_size,
                attention_dropout=attention_dropout,
                relu_dropout=relu_dropout,
                dropout=dropout,
    ...
    def forward(self, input, encoder_state, movie_embed_state, concept_embed_state, incr_state=None):
        # def forward(self, input, encoder_state, movie_embed_state, decoder_stage1_state, incr_state=None):
        encoder_output, encoder_mask = encoder_state
        movie_embed, movie_embed_mask = movie_embed_state
        concept_embed, concept_embed_mask = concept_embed_state
        # decoder_stage1_output, decoder_stage1_mask = decoder_stage1_state
        ...
        for layer in self.layers:
            tensor = layer(tensor, encoder_output, encoder_mask, movie_embed, movie_embed_mask, concept_embed, concept_embed_mask)
            # tensor = layer(tensor, encoder_output, encoder_mask, movie_embed, movie_embed_mask, decoder_stage1_output, decoder_stage1_mask)
